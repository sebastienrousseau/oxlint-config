...
screenshot_alt: "@sebastienrousseau/oxlint-config architecture and rule enforcement."
footer_note: "@sebastienrousseau/oxlint-config is part of the @sebastienrousseau/config suite. Published under Apache-2.0 OR MIT."
copyright: "© 2026 Sebastien Rousseau. Licensed under Apache-2.0 OR MIT."
translation_key: "home"
title: "@sebastienrousseau/oxlint-config — Shareable Oxlint configuration rules for ultra-fast Rust-based static code analysis."
description: "Shareable Oxlint configuration rules for ultra-fast Rust-based static code analysis."
keywords: "oxlint-config, oxlint, oxc, rust linter, fast linter, static analysis, javascript, typescript, developer tools, sebastien rousseau, standard"
hero_tag: "v0.0.3"
hero_tagline: "Ultra-fast Rust-based static analysis configuration"
headline: "@sebastienrousseau/oxlint-config"
lead: "Shareable Oxlint configuration rules for ultra-fast Rust-based static code analysis."
cta_primary: "Install @sebastienrousseau/oxlint-config"
...
snippet_caption: "Install @sebastienrousseau/oxlint-config"
snippet_code: "npm install --save-dev @sebastienrousseau/oxlint-config"
...
## What is @sebastienrousseau/oxlint-config?

**@sebastienrousseau/oxlint-config** is an enterprise-grade, shareable configuration for [Oxlint](https://github.com/sebastienrousseau/oxlint-config)—the high-performance JavaScript/TypeScript linter designed by the Oxc project, written in Rust. Designed to deliver vetted, deterministic defaults, this package eliminates manual setup boilerplate and establishes rigorous code quality standards across modern software development pipelines.

By adopting this configuration, engineering teams ensure unified consistency across local development, continuous integration, and automated deployments. The package adheres strictly to the OpenSSF Best Practices badge program (ID: 14514) and achieves top-tier security ratings via Scorecard audits.
...
## Why Developers Choose @sebastienrousseau/oxlint-config

Modern JavaScript and TypeScript repositories frequently suffer from configuration drift, fragmented standards, and maintenance overhead. **@sebastienrousseau/oxlint-config** directly solves these challenges:
...
Integrating `@sebastienrousseau/oxlint-config` into any new or existing repository requires only three simple steps.
...
### Step 2: Configure `oxlint.json`

Create or update your `oxlint.json` file at the root of your project to extend the shared configuration:

```json
{
  "plugins": ["correctness", "typescript", "unicorn"]
}
```

Learn how to customize specific rules and override settings in the [Usage Guide](usage/).

### Step 3: Run Oxlint in Your Workflow

Execute Oxlint directly or configure standard quality scripts in your `package.json`:

```bash
npx oxlint
```

Run ultra-fast static analysis across thousands of files in single-digit milliseconds.
...
### How does Oxlint compare to ESLint?
Oxlint runs 50-100x faster than ESLint without requiring AST plugin overhead, catching critical correctness bugs before running deeper pipelines.

### Can I override individual rules in my project?
Yes. Every configuration allows local overrides directly within your `oxlint.json` file or inline settings. Detailed override recipes are documented in the [Usage Guide](usage/).

### Is @sebastienrousseau/oxlint-config safe for production CI/CD?
