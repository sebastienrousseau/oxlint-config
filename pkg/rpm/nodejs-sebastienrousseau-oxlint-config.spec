...
%global npm_bare      oxlint-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-oxlint-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Oxlint configuration rules for 50x–100x faster Rust-powered JS/TS linting

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/oxlint-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/oxlint-config is a shareable configuration preset with no runtime
...
install -pm 0644 .oxlintrc.json %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
